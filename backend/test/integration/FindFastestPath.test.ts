...
import GraphGateway from '../../src/application/gateway/GraphGateway';
import FindFastestPath from '../../src/application/usecase/FindFastestPath';
import Graph from '../../src/domain/entity/Graph';
import GraphGatewayHttp from '../../src/infra/gateway/GraphGatewayHttp';
...
let usecase: FindFastestPath;
let deliveryRepository: DeliveryRepository;
let graphGateway: GraphGateway;
...
beforeAll(async function () {
  ...
  graphGateway = new GraphGatewayHttp(httpClient);
  usecase = new FindFastestPath(graphGateway, deliveryRepository);
});
...
test('should test if a given algorithm returns the fastest path', async function () {
  const stubGraphGateway = Sinon.stub(
    GraphGatewayHttp.prototype,
    'get'
  ).resolves(graph);
  const input = {
    start: 'A1',
  ...
  expect(result.getDistance()).toBe(44.11);
  expect(result.steps).toHaveLength(3);
  expect(result.steps.at(0)).toHaveProperty('start', 'A1');
  expect(result.steps.at(0)).toHaveProperty('end', 'A2');
  expect(result.steps.at(2)).toHaveProperty('start', 'A1');
  expect(result.steps.at(2)).toHaveProperty('end', 'B1');
  stubGraphGateway.restore();
});
