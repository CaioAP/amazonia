import DijkstraAlgorithm from '../../src/domain/entity/DijkstraAlgorithm';
...
test('should test if the Dijkstra Algorithm returns the fastest path', function () {
  const input = {
    start: 'A1',
    end: 'B2',
  };
  const algorithm = new DijkstraAlgorithm(graph);
  const result = algorithm.findPath(input.start, input.end);
  expect(result.distance).toBe(30.65);
  expect(result.path).toHaveLength(3);
  expect(result.path).toEqual(['A1', 'B1', 'B2']);
});
